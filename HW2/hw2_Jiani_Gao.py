...
class BetterIndex(object):
    ...
    def wildcard_search_or(self, text):
        ...
        for token in self.tokenize(text, is_search = True):
            ...
            result_files = result_files.union(self._documents[index] for index in crawl_tree(self._bt.root, search_token))
    ...
    def wildcard_search_and(self, text):
        ...
        for token in self.tokenize(text, is_search = True):
            ...
            if count == 1:
                ...
                result_files = result_files.intersection(self._documents[index] for index in crawl_tree(self._bt.root, search_token))
        return list(result_files)
